...
# ============================================================
# CALCULATE BOUNDARY EXTENTS FOR CLOSING WALLS
# ============================================================
# Calculate boundary based on hexagon positions
boundaryOffset = SIDE_LENGTH / 2

# For honeycomb pattern with 4x4 cells:
# Last row offset
lastRowOffset = h_spacing / 2 if (NUM_ROWS - 1) % 2 == 1 else 0

minX = 0 - SIDE_LENGTH  # Leftmost point of first column hexagons
maxX = (NUM_COLS - 1) * h_spacing + lastRowOffset + SIDE_LENGTH  # Rightmost point
minY = 0 - SIDE_LENGTH  # Bottom vertex of first row
maxY = (NUM_ROWS - 1) * v_spacing + SIDE_LENGTH  # Top vertex of last row

# ============================================================
# CREATE PERIMETER BOUNDARY WALLS (CLOSED SIDES)
# ============================================================
# Add boundary walls to the sketch before creating geometry
# Bottom wall
s.Line(point1=(minX - boundaryOffset, minY - boundaryOffset),
       point2=(maxX + boundaryOffset, minY - boundaryOffset))

# Top wall
s.Line(point1=(minX - boundaryOffset, maxY + boundaryOffset),
       point2=(maxX + boundaryOffset, maxY + boundaryOffset))

# Left wall
s.Line(point1=(minX - boundaryOffset, minY - boundaryOffset),
       point2=(minX - boundaryOffset, maxY + boundaryOffset))

# Right wall
s.Line(point1=(maxX + boundaryOffset, minY - boundaryOffset),
       point2=(maxX + boundaryOffset, maxY + boundaryOffset))
...
bottomY = minY - boundaryOffset
topY = maxY + boundaryOffset
